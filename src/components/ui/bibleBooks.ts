export interface BibleBook {
  ...
  name: string;
  /**
   * Chapter count — stable across virtually every translation (unlike verse
   * numbering, which genuinely varies by translation/versification and is
   * fetched live from YouVersion instead — see VersePicker).
   */
  chapters: number;
}

// USFM book codes, used directly in YouVersion's passage-by-reference lookup
// (e.g. "JHN.3.16"). See VerseOfTheDayService.GetPassageAsync on the backend.
...
export function findBook(code: string): BibleBook | undefined {
  return ALL_BOOKS.find((b) => b.code === code);
}
